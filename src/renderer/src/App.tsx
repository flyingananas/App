...
${openThreadsStr}`;
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800 font-sans">
      <header className="px-6 py-4 bg-white shadow-sm border-b border-slate-200 flex justify-between items-center z-20">
        <div className="flex items-center space-x-6">
          <h1 className="text-xl font-bold tracking-tight text-slate-900">Prompt D</h1>
          <select
        ...
        <div className="text-sm flex items-center space-x-6">
          <button onClick={deleteCurrentProject} className="text-slate-400 hover:text-red-600 transition-colors font-medium">Delete Project</button>
          <div className="flex items-center space-x-2 bg-slate-100 px-3 py-1 rounded-full">
            ...
            <span className="text-xs font-medium text-slate-600 uppercase tracking-wider">{dbStatus}</span>
          </div>
        </div>
      </header>
        ...
        {activeTab === 'outline' && (
          <div className="p-6 h-full overflow-y-auto max-w-4xl mx-auto w-full">
            <div className="space-y-8">
              <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Logged Items</h2>
              {Object.keys(groupedItems).map((type) => (
                <div key={type} className="space-y-3">
                  <h3 className="text-sm font-semibold tracking-wider uppercase text-slate-500">{type}</h3>
                  {groupedItems[type].map((item) => (
                    <div key={item.id} id={`item-${item.id}`} className={`p-4 rounded-xl shadow-sm border ${focusedItemId === item.id ? 'bg-indigo-50 border-indigo-300' : 'bg-white border-slate-200'} transition-colors duration-500`}>
                      <div className="text-slate-700 leading-relaxed">{item.content}</div>
                    </div>
                  ))}
                </div>
              ))}
        ...
        {activeTab === 'chat' && (
          <div className="flex flex-col h-full max-w-4xl mx-auto w-full">
            <div className="flex-1 overflow-y-auto p-6 space-y-6 flex flex-col">
              {chatMessages.map((msg) => {
                if (msg.type === 'suggestion') {
                  return (
                    <div key={msg.id} className="card p-5 bg-indigo-50/50 border-indigo-100 self-center w-full max-w-lg shadow-sm">
                      <div className="flex items-center space-x-2 mb-3">
                        ...
                        <h4 className="font-semibold text-indigo-900">AI Suggestion</h4>
                      </div>
                      <p className="text-sm text-slate-600 mb-4">{msg.suggestion.explanation}</p>
                      ...
                      {msg.isEditing ? (
                        ...
                        <div className="bg-white p-3 rounded-lg border border-slate-200 mb-4 shadow-sm flex items-start space-x-3">
                          <span className="font-mono text-xs uppercase bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded mt-0.5">[{msg.suggestion.type}]</span>
                          <span className="text-slate-800 text-sm leading-relaxed">{msg.suggestion.content}</span>
                        </div>
                      )}

                      <div className="flex items-center space-x-3">
                        <button onClick={() => handleSuggestionAction(msg.id, 'accept', msg.suggestion)} className="btn-primary py-1.5 px-4 text-sm">Accept</button>
                        {!msg.isEditing && (
                          <button onClick={() => handleSuggestionAction(msg.id, 'edit', msg.suggestion)} className="btn-secondary py-1.5 px-4 text-sm">Edit</button>
                        )}
                        <button onClick={() => handleSuggestionAction(msg.id, 'reject')} className="text-slate-500 hover:text-slate-700 font-medium text-sm px-2">Reject</button>
                      </div>
                    </div>
                  );
                } else if (msg.type === 'auto_applied') {
                  return (
                    <div key={msg.id} className="self-center flex items-center justify-between w-full max-w-lg bg-indigo-50 border border-indigo-100 p-3 rounded-lg shadow-sm">
                      <span className="text-sm text-indigo-800 font-medium">{msg.content}</span>
                      <button onClick={() => handleUndoAutoApply(msg.id, msg.suggestion)} className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 bg-white border border-indigo-200 px-3 py-1 rounded-md shadow-sm">Undo</button>
                    </div>
                  );
                }
                ...
                if (msg.type === 'system') {
                  return (
                    <div key={msg.id} className="p-3 rounded-xl text-sm bg-slate-100 text-slate-600 self-center max-w-lg w-full text-center shadow-sm border border-slate-200">
                      {msg.content}
                    </div>
                  );
                }
                ...
                if (msg.role === 'assistant') {
                  return (
                    <div key={msg.id} className="p-4 rounded-2xl rounded-tl-none text-sm bg-white border border-slate-200 text-slate-800 self-start max-w-[85%] whitespace-pre-wrap shadow-sm leading-relaxed">
                      {msg.content}
                    </div>
                  );
                }
                ...
                return (
                  <div key={msg.id} className="p-4 rounded-2xl rounded-tr-none text-sm bg-indigo-600 text-white self-end max-w-[85%] whitespace-pre-wrap shadow-sm leading-relaxed">
                    {msg.content}
                  </div>
                );
              })}
            ...
            {showCheckpointPrompt && (
              <div className="m-4 p-4 card bg-yellow-50 border-yellow-200 flex justify-between items-center shadow-md">
                <div className="flex items-center space-x-3">
                ...
                <div className="space-x-3">
                  <button onClick={() => setShowCheckpointPrompt(false)} className="text-sm font-medium text-yellow-700 hover:text-yellow-900">Skip</button>
                  <button onClick={handleCheckpointConfirm} className="bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-1.5 px-4 rounded-lg transition-colors shadow-sm text-sm">Confirm Checkpoint</button>
                </div>
              </div>
            )}
            ...
            <div className="p-6 bg-transparent">
              {lghState !== 'IDLE' && (
                <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg shadow-sm text-blue-800 text-sm font-medium flex items-center space-x-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                  <span><strong>Log Hours:</strong> {lghPrompt}</span>
                </div>
              )}
              {lghState === 'IDLE' && lghPrompt && (
                <div className="mb-3 p-2 text-sm text-emerald-600 font-medium italic">{lghPrompt}</div>
              )}
              <form onSubmit={handleInputSubmit} className="flex space-x-3 relative">
                <input
                  ref={commandInputRef}
                  type="text"
                  className="input-field flex-1 py-3 px-5 shadow-sm rounded-xl text-base"
                  placeholder="Type a message or command (e.g., 'drp: thought', 'lgh')... (Cmd/Ctrl + K)"
                  value={inputValue}
